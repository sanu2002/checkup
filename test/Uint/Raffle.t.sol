...
contract TestRaffle is Test, Script, CodeConstants {
    ...
    uint256 public constant LINK_BALANCE = 80 ether;
    ...
    function testfulfillrandomwordsafterperformupkeep(
        ...
        VRFCoordinatorV2_5Mock(vrfCoordinator).fulfillRandomWords(
            RandomRequestId,
            address(raffle)
        ); //in script we no need to manually add the request id and consumer
    ...
    function testFulfillRandomWordsPicksAWinnerResetsAndSendsMoney()  public skiptest  {
    ...
    // we will fail in late 2 test cased jsut because of the VRFCoordinatorV2_5Mock we are pretending that that that is a real 
    // We will fail in the last 2 test cases just because of the VRFCoordinatorV2_5Mock. We are pretending that it is a real VRFCoordinator.
    // When interacting with a real network, the VRFCoordinatorV2_5Mock will not be used. Instead, the actual VRFCoordinator will handle the requests.
    // Ensure that you have set up the VRF subscription and funded it correctly on the real network.
    // Also, make sure that the contract is a valid consumer of the VRF subscription.
